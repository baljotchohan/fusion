...
Lead dev: Python, FastAPI, Band SDK, LangGraph, agent logic, memory, MCP, Railway deploy.
...
├── Procfile                  ← Railway deploy (exists)
...
|---|---|---|
| `GOOGLE_API_KEY` | your Gemini key | Railway + local .env |
| `FEATHERLESS_API_KEY` | BOA26 credits key | Railway + local .env |
| `AIMLAPI_KEY` | your AI/ML API key | Railway + local .env |
| `GROQ_API_KEY` | optional fallback | Railway + local .env |
| `BAND_API_KEY` | from band.ai dashboard | Railway + local .env |
| `BAND_MOCK` | `true` local, `false` prod | Railway + local .env |
| `ALLOWED_ORIGINS` | Vercel URL | Railway only |
| `NEXT_PUBLIC_API_URL` | Railway URL | Vercel dashboard |
| `MAX_UPLOAD_MB` | `10` (default) | Railway optional |

**API keys NEVER go in GitHub. Railway/Vercel dashboard only.**
...
railway login
railway init
railway variables set GOOGLE_API_KEY=xxx BAND_API_KEY=xxx BAND_MOCK=false ...
railway up
# Get URL: https://fusion-production-xxxx.up.railway.app
```

### Vercel (Frontend)
```bash
cd frontend
echo "NEXT_PUBLIC_API_URL=https://fusion-production-xxxx.up.railway.app" > .env.local
vercel --prod
# Set NEXT_PUBLIC_API_URL in Vercel dashboard too
```
