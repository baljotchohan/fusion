...
async def force_partial_verdict(incident_id: str):
    ...
    if not inc:
        logger.error(f"Watchdog: Incident {incident_id} not found in memory graph.")
        return

    # Check if a final decision already exists
    if inc.get("final_decision"):
        logger.info(f"Watchdog: Incident {incident_id} already has a final decision. Skipping.")
        return
...
# FUSION INVESTMENT MEMO (PARTIAL VERDICT - WATCHDOG RECOVERED)

> [!WARNING]
> This decision was auto-generated by the pipeline watchdog because one or more
> agents stalled or timed out during the deliberation round.

### 1. PARTIAL DILIGENCE REPORT
Due to an unexpected pipeline stall, the investment committee has rendered a partial verdict based on the available partner findings.

### 2. PARTNER DILIGENCE SUMMARIES
* **Financial Partner:** {findings.get('financial_partner', 'Diligence report pending or timed out.')}
* **Legal Partner:** {findings.get('legal_partner', 'Diligence report pending or timed out.')}
* **Technical Partner:** {findings.get('technical_partner', 'Diligence report pending or timed out.')}
* **Market Partner:** {findings.get('market_partner', 'Diligence report pending or timed out.')}

---
— FUSION Swarm Committee Watchdog Recovery OS
"""
