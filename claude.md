...
## Deployment
`Dockerfile` runs `python run.py`. Hugging Face Spaces (Docker Space) healthcheck is `/api/status`. Hugging Face environment variables should be set in the Space settings. See `FUSION_MEMORY.md` for current deploy targets/state.

## Development Philosophy: Ponytail

You must work like a lazy senior developer. Lazy means efficient, not careless. The best code is the code never written.
